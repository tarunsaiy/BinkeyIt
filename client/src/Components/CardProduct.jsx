...
const CardProduct = ({ data }) => {
    ...
    return (
        <Link to={url} className='border border-slate-200 p-4 grid gap-2 max-w-48 min-w-48 h-65 rounded-2xl'>
            <div className='min-h-20 max-h-32 rounded relative'>
                {data.discount > 0 &&
                    (
                        <div className='bg-blue-500 text-white px-1 text-xs py-0.5 absolute -top-4 -left-1 flex items-center flex-col justify-center font-semibold shadow-md'>
                            <p className='text-xs'>
                                {data.discount}%
                            </p>
